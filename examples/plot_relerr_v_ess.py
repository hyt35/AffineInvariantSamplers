...
if __name__ == "__main__":
    ...
    ax.set_ylabel("ESS/grad")

    arr_relerr, arr_ess = [], []
    for ss in np.linspace(0.002,0.38,num=50):
        t0 = time.time()
        s, info = sampler_aldi(log_prob, init, n_samp, warmup=warmup,
                                step_size=ss, seed=seed, verbose=True, adapt=False)
        
        relerr, ess = _report("aldi", s, cov, info, time.time() - t0)
        arr_relerr.append(relerr)
        arr_ess.append(ess)
    ax.plot(arr_relerr, arr_ess, label="ALDI")
    
    arr_relerr, arr_ess = [], []
    for ss in np.linspace(0.01,0.9,num=50):
        t0 = time.time()
        s, info = sampler_pickles_unadjusted(log_prob, init, n_samp, warmup=warmup,
                                        step_size=ss, 
                                        seed=seed, verbose=True, adapt=False)
        relerr, ess = _report("pickles_unadjusted", s, cov, info, time.time() - t0)
        arr_relerr.append(relerr)
        arr_ess.append(ess)
    ax.plot(arr_relerr, arr_ess, label="Unadj. PICKLES")

    arr_relerr, arr_ess = [], []
    for ss in np.linspace(0.1,0.8,num=50):
        t0 = time.time()
        s, info = sampler_pickles(log_prob, init, n_samp, warmup=warmup,
                                        step_size=ss, 
                                        seed=seed, verbose=True, adapt_L=False, adapt_step_size=False, L=1, find_init_step_size=False)
        relerr, ess = _report("pickles", s, cov, info, time.time() - t0)
        arr_relerr.append(relerr)
        arr_ess.append(ess)
    ax.plot(arr_relerr, arr_ess, label="Metr. PICKLES L=1")

    ax.legend()
    ax.set_yscale("log")
    fig.savefig("figs/gaussian_relerr_v_ESS.pdf")
    ...
    #     fig.savefig(f'figs/gaussian_{name}.png')
    #     fig.savefig(f'figs/gaussian_{name}.pdf')
    # plt.show()
